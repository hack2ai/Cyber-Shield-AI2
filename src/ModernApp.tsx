...
function localFallback(target: string): ThreatOverviewResult {
  ...
  const riskIndicators = [...keywordHits.map(item => `PHISHING_KEYWORD: ${item}`), ...(isShortener ? ['URL_SHORTENER_DETECTED'] : []), ...(isPunycode ? ['PUNYCODE_HOMOGRAPH_RISK'] : [])];
  return {
    threatScore, classification, target, type: 'target',
    ...
    riskIndicators,
    technicalSummary: { dns: 'Server DNS evidence unavailable in fallback mode.', ssl: 'Server TLS evidence unavailable in fallback mode.', threatIntel: 'Client-side heuristic engine active.' },
...
export default function ModernApp() {
  ...
  const statusText = useMemo(() => isAnalyzing ? 'Investigation running' : result ? 'Investigation complete' : 'Engine ready', [isAnalyzing, result]);
  ...
  const analyze = async () => {
    ...
    try {
      const response = await fetch('/api/analyze', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: value }) });
      if (!response.ok) {
  ...
  if (showLegacy) return <div className="relative min-h-screen"><button onClick={() => setShowLegacy(false)} className="fixed left-4 top-4 z-[120] inline-flex items-center gap-2 rounded-lg border border-sky-400/20 bg-slate-950/90 px-3 py-2 text-xs font-medium text-sky-300 shadow-xl backdrop-blur-xl hover:bg-slate-900"><ArrowLeft size={14} /> Back to SOC workspace</button><LegacyApp /></div>;
    ...
    <div className="min-h-screen px-4 py-5 text-slate-100 md:px-6 lg:px-8">
      <div className="mx-auto max-w-[1500px]">
        <header className="mb-6 flex flex-col gap-4 border-b border-white/[0.07] pb-5 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl border border-sky-400/20 bg-sky-400/[0.06] text-sky-300 shadow-[0_10px_30px_rgba(14,165,233,.08)]"><Shield size={22} /></div><div><div className="flex items-center gap-2"><h1 className="text-lg font-semibold tracking-tight">Cyber Shield AI</h1><span className="status-badge">SOC workspace</span></div><p className="mt-1 text-xs text-slate-500">AI-assisted threat intelligence and defensive indicator analysis</p></div></div>
          <div className="flex flex-wrap items-center gap-2"><div className="status-badge"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />{statusText}</div><button onClick={() => setShowLegacy(true)} className="inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2 text-xs font-medium text-slate-400 hover:border-sky-400/20 hover:text-slate-200"><Layers3 size={14} /> Full console</button></div>
        </header>
